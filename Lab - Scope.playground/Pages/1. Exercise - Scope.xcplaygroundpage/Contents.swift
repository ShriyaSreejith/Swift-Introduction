...
//print("The value of foo is \(foo)")
// This will generate an error because foo is a local variable and therefore can't be accessed outside of that function.
...
//Both these statements will run because x is defined both outside the function and inside(global and local respectively) So when x is printed inside, its showing the updated x value while the outside print statement prints the old x value. foo is defined and used as a local variable.
