...
calculateArea(x: 2.4, y: 12.8)
calculateArea(x: -3.4, y: -7.4)
...
func createUser() -> User?{
    guard let firstNameUnwrapped = firstNameTextField.text, let lastNameUnwrapped = lastNameTextField.text, let ageUnwrapped = ageTextField.text else{
        return nil
    }
    return User(firstName: firstNameUnwrapped, lastName: lastNameUnwrapped, age: ageUnwrapped)
}
...
if let user = createUser(){
    print("The user first name is \(user.firstName)")
    print("The user last name is \(user.lastName)")
    print("The user age is \(user.age)")
}
